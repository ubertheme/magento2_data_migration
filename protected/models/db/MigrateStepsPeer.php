...
class MigrateStepsPeer extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('title, code, status, sorder', 'required'),
			array('status, sorder', 'numerical', 'integerOnly'=>true),
			array('title, code', 'length', 'max'=>255),
			array('migrated_data, descriptions', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, code, status, migrated_data, descriptions, sorder', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('migrated_data',$this->migrated_data,true);
